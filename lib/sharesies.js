...
function printFundInvestmentInfo(fund, marketPricesNormalized, investmentAmount) {
    ...
    let range = `${cprint.toCyan('$' + info.minPrice.toFixed(2))} ${cprint.toLightGrey('<==')} ${rangePositionColorFn('$' + info.currentPrice.toFixed(2) + ' [' + info.currentNormalizedPrice.toFixed(2) * 100 + '%' + ']')} ${cprint.toLightGrey('==>')} ${cprint.toCyan('$' + info.maxPrice.toFixed(2))}`;
    ...
    if (potentialReturn > 0) {
        invest = `${cprint.toWhite(', invest:')} ${cprint.toCyan('$' + investmentAmount.toFixed(2))} ${cprint.toLightGrey('for approx. return')} ${returnColorFn('$' + potentialReturn.toFixed(2) + ' - $' + maxReturn.toFixed(2))}`;
    }
