            ...
            new Promise((resolve, reject) =>
                request.get('fund/list', {}).then(
                    (data) => {
                        ...
                        let funds = data['funds'].filter((fund) => ['NZB', '450002', '450007'].indexOf(fund.code) < 0);
...
function printFundInvestmentInfo(fund, marketPricesNormalized) {
    ...
    let infoRange = `${cprint.toCyan(('$' + info.minPrice.toFixed(2)).leftPad(' ', 5))} ${lightGreyFn('<==')} ${rangePositionColorFn(
        ('$' + info.currentPrice.toFixed(2)).leftPad(' ', 5) + ' [' + rangePercentageStr + ']'
    )} ${lightGreyFn('==>')} ${cprint.toCyan(('$' + info.maxPrice.toFixed(2)).leftPad(' ', 5))}`;
    ...
    let differenceStr = ('$' + priceRange.toFixed(2)).leftPad(' ', 6) + ' (' + priceDiffStr + '%)';
