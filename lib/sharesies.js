...
function getFundsCleaned() {
    return getFundsFromStore().then(funds => {
        let dateKeys = dateExt.getDateStampRange(350).reverse();
...
function printFundInvestmentInfo(fund, marketPricesNormalized, investmentAmount) {
    ...
    if (potentialReturn > 0) {
        let potentialReturnStr = ('$' + potentialReturn.toFixed(2)).rightPad(' ', 7);
        let maxReturnStr = ('$' + maxReturn.toFixed(2)).leftPad(' ', 7);
        invest = `${cprint.toWhite('invest:')} ${cprint.toCyan('$' + investmentAmount.toFixed(2))} ${lightGreyFn('for approx. return')} ${returnColorFn(potentialReturnStr + ' - ' + maxReturnStr)}`;
    }

    process.stdout.write(`${cprint.toGreen(code)}    ${range}    ${difference}    ${score}    ${invest}\n`);
}
