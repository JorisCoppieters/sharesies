...
function getFundsCleaned() {
    return _getFundsFromStore().then(funds => {
...
function getFundInvestmentInfo(fund, marketPricesNormalized) {
    ...
    let score = 0;
    score += (currentNormalizedMarketPrice - currentNormalizedPrice) * 2.5; // Weight towards how much below the market line current price is
        ...
        return new Promise((resolve, reject) => {
            fs.readFile(storeFile, (err, data) => {
                ...
                if (!jsonData.date || (jsonData.date < tenMinutesAgo)) {
                    ...
                    return _getFundsFromStore().then(resolve);
                }
