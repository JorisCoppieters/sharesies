...
function printFundInvestmentInfo(fund, marketPricesNormalized) {
    ...
    let infoRange = `${cprint.toCyan('$' + info.minPrice.toFixed(2))} ${lightGreyFn('<==')} ${rangePositionColorFn('$' + info.currentPrice.toFixed(2) + ' [' + rangePercentageStr + ']')} ${lightGreyFn('==>')} ${cprint.toCyan('$' + info.maxPrice.toFixed(2))}`;
