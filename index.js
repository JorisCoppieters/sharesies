...
sync.runGenerator(function* () {
    try {
        ...
        let sellingSharesValue = sharesiesInfo.orders
            ...
            .map((order) => {
                let fund = funds.find((fund) => fund.id === order['fund_id']);
                if (!fund) {
        ...
        let exploratoryInvestmentScore = sortedFundsByBuy.length
            ? sortedFundsByBuy
                  .map((fundInfo) => {
                      return Math.pow(fundInfo.info.score * 0.6, DISTRIBUTION_MAGNITUDE);
                  })
                  .reduce((scoreSum, score) => scoreSum + score, 0) / sortedFundsByBuy.length
            : 0;
        ...
        let remainingBalanceStr =
            cprint.toCyan('<--') + ' ' + cprint.toWhite('$' + remainingBalance.toFixed(2)) + ' ' + cprint.toCyan('-->');
        print.info(
            `${cprint.toYellow(
                'Exploratory'
            )} ${exploratoryBallanceStr} ${remainingBalanceStr} ${diversificationBallanceStr} ${cprint.toGreen('Diversification')}`
        );
        ...
        if (SELL_LOCK) {
            fundsFound = 0;
            sellingSharesValue = 0;
            fundsSaleAllocation = 0;
        }
        ...
        if (exploratoryInvestmentBalance > 0) {
            ...
            if (fundsSaleAllocation > 0) {
                if (fundsSaleAllocation < 100) {
                    print.info(
                        `Allocated $${fundsFound.toFixed(2)} for exploratory investment, so that is close enough, no need to sell anything`
                    );
                    yield sellShares(user, sharesiesInfo, sortedFunds, 0);
                    sharesiesInfo = yield sharesies.getInfo();
                } else {
                    print.info(
                        `Allocated only $${fundsFound.toFixed(
                            2
                        )} for exploratory investment, so need to sell $${fundsSaleAllocation.toFixed(
                            2
                        )} for further exploratory investment`
                    );
                ...
                print.info(
                    `Allocated more than enough $${fundsFound.toFixed(
                        2
                    )} for exploratory investment (including selling $${sellingSharesValue.toFixed(2)}), no need to sell more`
                );
            } else {
        ...
        let maxInvestmentFundCodes = maxInvestmentStrategy.bestValueIncreases
            ...
            .filter((_, idx) => idx < 3)
            .map((fund) => `${fund.fundCode} (${fund.fundName}) [x${parseInt(fund.fundMultiplierAtIdx * 1000) / 1000}]`)
            .join(', ');
        ...
        // sharesiesStats.total_deposits = sharesiesTransactions
        //     .filter(txn => txn.description === "Deposit")
        //     .reduce((total, txn) => total + parseInt(txn.amount), 0);

        // sharesiesStats.total_withdrawals = sharesiesTransactions
        //     .filter(txn => txn.description === "Withdrawal")
        //     .reduce((total, txn) => total + -parseInt(txn.amount), 0);
        ...
        print.info(`Best invested value increases: ${lightBlueFn(investmentFundGainCodes)}`);
        print.info(`Worst invested value decreases: ${lightYellowFn(investmentFundLossCodes)}`);
        ...
        .then(
            sortedFunds
                ...
                .forEachThen((fundInfo, idx) => {
                    ...
                    _debug(`Fund: ${fundInfo.fund.code} (${fundInfo.fund.name})`);
...
function getInvestmentStrategy(sharesiesTransactions, sharesiesInfo, sortedFunds, daysAgo) {
    ...
    let fundAmount = sharesiesInfo.funds.reduce((dict, fund) => {
        let fundCode =
            sortedFunds.filter((sortedFund) => sortedFund.fund.id === fund.fund_id).map((sortedFund) => sortedFund.fund.code)[0] ||
            fund.fund_id;
    ...
    for (let dayAgoIdx = 0; dayAgoIdx < daysAgo; ++dayAgoIdx) {
        ...
        sortedFunds.forEach((sortedFund) => {
            let fundMultiplierAtIdx =
                sortedFund.info.fundPrices[fundPriceHistoryIdx] / sortedFund.info.fundPrices[fundPriceHistoryIdx - 1] || 1;
        ...
        bestValueIncreases.push({
            ...
            fundName: maxFund ? maxFund.fund.name : '',
        ...
        worstValueIncreases.push({
            ...
            fundName: minFund ? minFund.fund.name : '',
    ...
    return {
        bestValueIncreases: bestValueIncreases.sort((a, b) => b.fundReturn - a.fundReturn),
        worstValueIncreases: worstValueIncreases.sort((a, b) => a.fundReturn - b.fundReturn),
...
function getMaxInvestmentStrategy(sortedFunds, daysAgo) {
    ...
    return {
        bestValueIncreases: bestValueIncreases.sort((a, b) => b.fundMultiplierAtIdx - a.fundMultiplierAtIdx),
    };
}
...
function _debug(in_data) {
    if (DEBUG) {
        process.stdout.write(cprint.toDarkGray(`*** DEBUG *** ${JSON.stringify(in_data)}\n`));
    }
    return in_data;
}
